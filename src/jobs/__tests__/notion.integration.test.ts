...
describe('Jobs fetch api', () => {
    it('should fetch and return a list of job', async () => {
        mockedAxios.post.mockImplementation(() => Promise.resolve({ data: jobsStub }));

        const { jobs } = await NotionService.all({});
    ...
    it('should fetch and return one job details', async () => {
        mockedAxios.get.mockImplementation((url) => Promise.resolve({
