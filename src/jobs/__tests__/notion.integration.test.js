...
describe('Jobs fetch api', () => {
    ...
    it('should fetch and return one job details', async () => {
        ...
        expect(result.teamInfo).toEqual('Catherine BRAX, cheffe du bureau des systèmes d’information de gestion et du décisionnel');
        expect(result.toApply).not.toBeFalsy();
        expect(result.slug).toEqual('data-scientist-fh-acd471f0-2db5-4685-bdb7-eeaba1f03875');
        expect(result.department).toEqual(['Ministère de l’éducation nationale de la jeunesse et des sports – Ministère de l’enseignement supérieur de la recherche et de l’innovation']);
        ...
        expect(result.team).toEqual('Au sein de la sous-direction des services numériques, vous intégrerez une équipe de 23 personnes (chefs de projets nationaux et concepteurs-développeurs) dédiée aux systèmes d\'information de gestion et du décisionnel.');
    });
});
