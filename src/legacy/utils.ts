  ...
  const properties: any = Object.keys(pepJob).reduce((acc: any, property) => {
    if (
      ...
      acc[`${property}Formated`] = {
        date: {
          start:
            parse(pepJob[property], 'dd/MM/yyyy HH:mm:ss', new Date()) ||
            parse(pepJob[property], 'dd/MM/yyyy h:mm:ss', new Date()),
        },
      }
    }
...
export const dateReadableFormat = (date, now = new Date()) => {
  try {
    ...
    const humanizedTimeAgo = capitalize(formatDistance(dateInstance, now, { addSuffix: true, locale: fr }))
